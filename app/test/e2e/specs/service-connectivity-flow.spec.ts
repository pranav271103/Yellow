...
const mockStateFile =
  process.env.Yellow_SERVICE_MOCK_STATE_FILE ||
  path.join(process.env.Yellow_WORKSPACE || os.tmpdir(), 'service-mock-state.json');
...
describe('Service connectivity flow (UI ↔ Rust service)', () => {
  before(async function beforeSuite() {
    if (process.env.Yellow_SERVICE_MOCK !== '1' || process.platform === 'linux') {
      this.skip();
    }

    stepLog('Starting suite with service mock mode enabled', {
      YellowServiceMock: process.env.Yellow_SERVICE_MOCK,
      mockStateFile,
    });
  ...
  it('shows the blocking gate when service is not installed', async () => {
    await waitForText('Yellow Service Required', 20_000);
  ...
  it('keeps user blocked and surfaces error when core start fails', async () => {
    ...
    await waitForText('Yellow Service Required', 10_000);
  ...
  it('unblocks once service is running and agent is reported healthy', async () => {
    ...
    await browser.waitUntil(async () => !(await textExists('Yellow Service Required')), {
      timeout: 20_000,
  ...
  it('shows blocking gate again when connection suddenly breaks', async () => {
    ...
    await browser.waitUntil(async () => textExists('Yellow Service Required'), {
      timeout: 20_000,
      interval: 500,
